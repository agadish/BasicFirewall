...
proxy_entry_create(proxy_connection_entry_t **pentry_out)
{
    ...
    pentry->client_conn = (proxy_connection_t *)\
          kmalloc(sizeof(*pentry->client_conn), GFP_KERNEL);
    ...
    pentry->server_conn = (proxy_connection_t *)\
          kmalloc(sizeof(*pentry->server_conn), GFP_KERNEL);
                                       ...
                                       const struct sk_buff *skb)
{
    ...
    if (CONNECTION_does_id_match_skb(&pentry->client_conn->opener.id, skb)) {
        /* Client to proxy */
        result = PACKET_DIRECTION_FROM_CLIENT;
    } else if (CONNECTION_does_id_match_skb(&pentry->client_conn->listener.id,
                                            skb)) {
        /* Server to proxy */
        result = PACKET_DIRECTION_FROM_SERVER;
    }
                         ...
                         const struct sk_buff *skb)
{
    ...
    is_src_ip_from_localhost = ((ip_header->saddr == local_ip) ||
                                (0 == local_ip));
    is_src_port_match = (tcp_header->source == pentry->client_conn->proxy_port);
    is_dst_ip_match = (ip_header->daddr ==
                       pentry->client_conn->listener.id.dst_ip);

    is_dst_port_match = (tcp_header->dest ==
                         pentry->client_conn->listener.id.dst_port);
                         ...
                         const struct sk_buff *skb)
{
    ...
    is_src_port_misconfigured = (
        (0 == pentry->server_conn->proxy_port) &&
        (TCP_CLOSE == pentry->server_conn->opener.state)
    );

    is_src_port_match = tcp_header->source == pentry->server_conn->proxy_port;
    is_dst_ip_match = (
        ip_header->daddr == pentry->server_conn->listener.id.src_ip
    );
...
{
    if (NULL == conn) {
        (void)memset(g_singleconn_string_buff,
                     0,
                     sizeof(g_singleconn_string_buff));
    } else {
...
{
    size_t dumped_size = 0;
    const size_t required_size = (sizeof(*pentry->client_conn_nonproxy) +
                                  sizeof(*pentry->server_conn_nonproxy));
    ...
    (void)memcpy(buffer,
                 pentry->client_conn_nonproxy,
                 sizeof(*pentry->client_conn_nonproxy));

    (void)memcpy(&buffer[sizeof(*pentry->client_conn_nonproxy)],
                 pentry->server_conn_nonproxy,
                 sizeof(*pentry->server_conn_nonproxy));
    ...
    if ((NULL != src_out) && (NULL != dst_out)) {
        ...
        case PACKET_DIRECTION_TO_SERVER:
            ...
            *dst_out = &entry->server_conn->listener;
            break;
        default:
            printk(KERN_ERR "%s: error invalid direction\n", __func__);
            is_success = FALSE;
            break;
        }
    } else {
        is_success = FALSE;
        printk(KERN_ERR "%s: something is BAD\n", __func__);
    }
